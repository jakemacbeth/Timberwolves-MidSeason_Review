...
from src.etl.error_handler import log_etl_error
    ...
    for i, r in enumerate(rows, start=1):
        ...
        try:
            ...
            logger.info(f"[{i}/{len(rows)}] Loaded playerbox_pergame for game_id={game_id} (rows={len(box_rows)})")
                    ...
                    failed += 1
                    logger.error(f"[{i}/{len(rows)}] ERROR loading playerbox_pergame for game_id={game_id}: {e}")
                    log_etl_error(                          
                        engine=engine,
                        process_name="load_dimplayer_boxscores",
                        error=e,
                        game_id=game_id
                    )
